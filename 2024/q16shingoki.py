...
def plot_numbers_on_grid(black, white, shingoki_with, shingoki_height, illegal_combinations=None):
    # Define grid dimensions
    x_lines = np.arange(0, shingoki_with + 1)  # 10 vertical lines
    y_lines = np.arange(0, shingoki_height + 1)   # 5 horizontal lines
    ...
    for x in range(0, shingoki_with + 1):
        ax.plot([x, x], [0, shingoki_height], color='black', linewidth=0.5)  # Vertical lines

    for y in range(0, shingoki_height + 1):
        ax.plot([0, shingoki_with], [y, y], color='black', linewidth=0.5)  # Horizontal lines
    ...
    if illegal_combinations is not None:
    ...
    # Set grid limits
    ax.set_xlim(-1, shingoki_with + 1)
    ax.set_ylim(-1, shingoki_height + 1)
...
if __name__ == '__main__':
    ...
    plot_numbers_on_grid(black, white, 10, 5, lines)
    ...
    letters = [list(message[i:i + 10]) for i in range(0, len(message), 10)]
